    ...
    for (; iter; ++iter) {
        if (iter->type == COMMITTED) {
            ...
            case SNAPSHOT_SAVE: {
                ...
                const bool bool_ret = caller->pass_by_status(iter->done);
                LOG(INFO)<<"FSMCaller run SNAPSHOT_SAVE bool_ret "<<bool_ret;
                if (bool_ret) {
                    caller->do_snapshot_save((SaveSnapshotClosure*)iter->done);
                }
                break;
            }
            case SNAPSHOT_SAVE_WITH_META: {
                caller->_cur_task = SNAPSHOT_SAVE_WITH_META;
                const bool bool_ret = caller->pass_by_status(iter->done);
                LOG(INFO)<<"FSMCaller run SNAPSHOT_SAVE_WITH_META bool_ret "<<bool_ret;
                if (bool_ret) {
                    caller->do_snapshot_save_with_meta((SaveSnapshotClosure*)iter->done);
                }
                break;
            }
            case SNAPSHOT_LOAD:
...
int FSMCaller::init(const FSMCallerOptions &options) {
    ...
    LOG(INFO) << "FSMCaller init _last_applied_index " << options.bootstrap_id.index 
      << " term " << options.bootstrap_id.term ;
    _queue_started = true;
    return 0;
}
...
int FSMCaller::on_snapshot_save_with_meta(SaveSnapshotClosure* done) {
    LOG(INFO)<<"FSMCaller Node "<<_node->node_id() <<" on_snapshot_save_with_meta";
    ApplyTask task;
    task.type = SNAPSHOT_SAVE_WITH_META;
    task.done = done;
    return bthread::execution_queue_execute(_queue_id, task);
}

int FSMCaller::on_snapshot_save(SaveSnapshotClosure* done) {
    LOG(INFO)<<"FSMCaller Node "<<_node->node_id() <<" on_snapshot_save";
    ApplyTask task;
...
void FSMCaller::get_snapshot_meta(SnapshotMeta &meta) {
...
void FSMCaller::do_snapshot_save(SaveSnapshotClosure* done) {
    ...
    if (!writer) {
        ...
        LOG(ERROR)<<"snapshot_storage create SnapshotWriter failed";
        return;
    }

    LOG(INFO)<<"FSMCaller Node "<<_node->node_id() <<" do_snapshot_save";
    _fsm->on_snapshot_save(writer, done);
    return;
}

void FSMCaller::do_snapshot_save_with_meta(SaveSnapshotClosure* done) {
    CHECK(done);

    SnapshotMeta &meta = done->get_snapshot_meta();

    ConfigurationEntry conf_entry;
    _log_manager->get_configuration(meta.last_included_index(), &conf_entry);
    for (Configuration::const_iterator
            iter = conf_entry.conf.begin();
            iter != conf_entry.conf.end(); ++iter) { 
        *meta.add_peers() = iter->to_string();
    ...
    SnapshotWriter* writer = done->get_snapshot_writer();
    if (!writer) {
        done->status().set_error(EINVAL, "snapshot_storage create SnapshotWriter failed");
        done->Run();
        LOG(ERROR)<<"snapshot_storage create SnapshotWriter failed";
    ...
    LOG(INFO)<<"FSMCaller Node "<<_node->node_id() <<" do_snapshot_save_with_meta";
    _fsm->on_snapshot_save_with_meta(writer, done);
    return;
}
