...
int SegmentLogStorage::init(ConfigurationManager* configuration_manager) {
    ...
    if (butil::crc32c::IsFastCrc32Supported()) {
        ...
        LOG_ONCE(INFO) << "SegmentLogStorage Use crc32c as the checksum type of appending entries";
    } else {
        _checksum_type = CHECKSUM_MURMURHASH32;
        LOG_ONCE(INFO) << "SegmentLogStorage Use murmurhash32 as the checksum type of appending entries";
    }
    ...
    LOG(INFO) << "SegmentLogStorage init ret:" << ret;
    return ret;
}
...
int SegmentLogStorage::list_segments(bool is_empty) {
    ...
    if (!dir_reader.IsValid()) {
        ...
        LOG(INFO) << "list_segments directory reader path: " << _path;
    }
