...
int NodeImpl::init_meta_storage() {
    ...
    LOG(INFO) << "node " << _group_id << ":" << _server_id << " url " << _options.raft_meta_uri << " ret " << ret;
...
int NodeImpl::init(const NodeOptions& options) {
    ...
    if (!st.ok()) {
        ...
        LOG(INFO) << "node " << _group_id << ":" << _server_id
                   << " is initialized with consitency log: "
                   << st;
    }
    ...
    if (_options.is_learner) {
     ...
     LOG(INFO) << "node " << _group_id << ":" << _server_id
               << " will set state:" << state2str(_state);
    ...
    if (!NodeManager::GetInstance()->add(this)) {
        ...
        LOG(INFO) << "NodeManager add " << _group_id 
                   << ":" << _server_id << " success";
    }
    ...
    if (_conf.stable() && _conf.conf.size() == 1u
        ...
        if (true == get_can_elect_to_leader()) {
          LOG(INFO) << "raft node will elect_self";
          elect_self(&lck);
        } else {
          LOG(INFO) << "raft node will not elect_self";
        }
    }
...
void NodeImpl::snapshot(const SnapshotMeta &meta, Closure* done) {
    do_snapshot(meta, done);
}

void NodeImpl::do_snapshot(Closure* done) {
    LOG(INFO) << "node " << _group_id << ":" << _server_id 
              << " starts to do snapshot _snapshot_executor "<<_snapshot_executor;
    if (_snapshot_executor) {
        _snapshot_executor->do_snapshot(done);
    } else {
        if (done) {
...
void NodeImpl::do_snapshot(const SnapshotMeta &meta, Closure* done) {
    LOG(INFO) << "node " << _group_id << ":" << _server_id << " meta " << meta.DebugString() 
              << " starts to do snapshot _snapshot_executor "<<_snapshot_executor;
    if (_snapshot_executor) {
        _snapshot_executor->do_snapshot(meta, done);
    } else {
        if (done) {
            done->status().set_error(EINVAL, "Snapshot is not supported");
            run_closure_in_bthread(done);
...
void NodeImpl::get_snapshot_meta(SnapshotMeta &meta) {
    if (_snapshot_executor) {
        _snapshot_executor->get_snapshot_meta(meta);
        LOG(INFO) << "node " << _group_id << ":" << _server_id <<  " get snapshot meta: "
                  << meta.DebugString();
    } else {
    }
}
...
void NodeImpl::handle_election_timeout() {
    ...
    LOG(INFO) << "handle_election_timeout";
    return pre_vote(&lck);
...
void NodeImpl::handle_timeout_now_request(brpc::Controller* controller,
    ...
    LOG(INFO) << "handle_timeout_now_request elect_self";
    elect_self(&lck);
...
void NodeImpl::handle_vote_timeout() {
	...
	LOG(INFO) << "handle_vote_timeout elect_self";
        elect_self(&lck);
    }
}
...
void NodeImpl::handle_pre_vote_response(const PeerId& peer_id, const int64_t term,
    ...
    if (response.granted()) {
	    ...
	    LOG(INFO) << "handle_pre_vote_response elect_self";
            elect_self(&lck);
        }
    }
}
...
void NodeImpl::pre_vote(std::unique_lock<raft_mutex_t>* lck) {
    ...
    if (_pre_vote_ctx.granted()) {
      if (true == get_can_elect_to_leader()) {
        LOG(INFO) << "pre_vote elect_self";
        elect_self(lck);
      } else {
        LOG(INFO) << "pre_vote not elect_self";
      }
    }
}
...
void NodeImpl::unsafe_get_status(NodeStatus* status) {
	if (status == NULL) {
		return;
	}
	...
	_log_manager->unsafe_get_status(&log_manager_status);
