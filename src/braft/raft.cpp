...
int Node::init(const NodeOptions& options, bool can_elect_to_leader) {
    set_can_elect_to_leader(can_elect_to_leader);
    return _impl->init(options);
}

void Node::set_can_elect_to_leader(const bool can_elect_to_leader) {
  _impl->set_can_elect_to_leader(can_elect_to_leader);
}

bool Node::get_can_elect_to_leader() const {
  return _impl->get_can_elect_to_leader();
}
...
void Node::snapshot(const SnapshotMeta &meta, Closure* done) {
    _impl->snapshot(meta, done);
}

void Node::get_snapshot_meta(SnapshotMeta &meta) {
  _impl->get_snapshot_meta(meta);
}
...
void Node::unsafe_get_status(NodeStatus* status) {
    return _impl->unsafe_get_status(status);
}
...
void StateMachine::on_snapshot_save_with_meta(SnapshotWriter* writer, Closure* done) {
    (void)writer;
    CHECK(done);
    LOG(ERROR) << butil::class_name_str(*this)
               << " didn't implement on_snapshot_save_with_meta";
    done->status().set_error(-1, "%s didn't implement on_snapshot_save_with_meta",
                                 butil::class_name_str(*this).c_str());
    done->Run();
}
