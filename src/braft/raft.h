...
public:
    ...
    virtual void on_snapshot_save_with_meta(::braft::SnapshotWriter* writer,
                                            ::braft::Closure* done);
...
inline const char* state2str(State state) {
    const char* str[] = {"LEADER", "TRANSFERRING", "CANDIDATE", "FOLLOWER", 
                        ...
                        "LEARNER", "ERROR", "UNINITIALIZED", "SHUTTING", "SHUTDOWN", };
    if (state < STATE_END) {
...
public:
    ...
    // init node
    int init(const NodeOptions& options, bool can_elect_to_leader = true);
    ...
    void set_can_elect_to_leader(const bool can_elect_to_leader);
    bool get_can_elect_to_leader() const;
