...
int LocalSnapshotMetaTable::remove_file(const std::string& filename) {
    ...
    LOG(INFO) << "snapshot meta table remove " << filename << " success";
    return 0;
}
...
int LocalSnapshotMetaTable::save_to_file(FileSystemAdaptor* fs, const std::string& path) const {
    ...
    LOG_IF(INFO, ret == 0) << "save meta to " << path << " success";
    return ret;
}
...
int LocalSnapshotMetaTable::load_from_file(FileSystemAdaptor* fs, const std::string& path) {
    ...
    LOG(INFO) << "load snapshot meta table from " << path << " success";
    return 0;
}
    ...
    if (snapshots.size() > 0) {
        ...
        for (size_t i = 0; i < snapshot_count - 1; i++) {
            ...
            if (!_fs->delete_file(snapshot_path, true)) {
                ...
                LOG(INFO) << "delete old snapshot path success, path " << snapshot_path;
            }
        }
...
void LocalSnapshotStorage::ref(const int64_t index) {
    ...
    _ref_map[index]++;
    LOG(INFO) << "ref snapshot success snapshot_index:" << index << " current_ref_count:" << _ref_map[index];
}
...
int LocalSnapshotStorage::destroy_snapshot(const std::string& path) {
    ...
    LOG(INFO) << "delete old snapshot path success, path " << path;
...
void LocalSnapshotStorage::unref(const int64_t index) {
    ...
    if (it != _ref_map.end()) {
        ...
        LOG(INFO) << "unref snapshot success snapshot_index:" << index << " current_ref_count:" << it->second;
        if (it->second == 0) {
            _ref_map.erase(it);
            lck.unlock();
