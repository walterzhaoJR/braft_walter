...
public:
    ...
    // trigger snapshot
    void snapshot(const SnapshotMeta &meta, Closure* done);
    ...
    //Get some internal status of this node
    //not with any lock
    void unsafe_get_status(NodeStatus* status);
    ...
    void set_can_elect_to_leader(const bool can_elect_to_leader) { _can_elect_to_leader = can_elect_to_leader; }
    bool get_can_elect_to_leader() const { return _can_elect_to_leader; }
    void get_snapshot_meta(SnapshotMeta &meta); 
...
friend class butil::RefCountedThreadSafe<NodeImpl>;
    ...
    void do_snapshot(const SnapshotMeta &meta, Closure* done);
...
private:
    ...
    bool _can_elect_to_leader;//for signal node satrt,we don't want it to become a leader.walter
};
