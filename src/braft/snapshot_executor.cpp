...
public:
    ...
    SnapshotMeta& get_snapshot_meta() { return _meta; }
    SnapshotWriter* get_snapshot_writer() { return _writer; }
...
void SnapshotExecutor::get_snapshot_meta(SnapshotMeta &meta) {
    _fsm_caller->get_snapshot_meta(meta);
}

void SnapshotExecutor::do_snapshot(const SnapshotMeta &meta, Closure* done) {
    ...
    if (!writer) {
        ...
        LOG(INFO) << "node " << (_node != NULL ? _node->node_id().to_string()  : "NULL")
            << " create SnapshotWriter success, "
            << " last_snapshot_index " << saved_last_snapshot_index
            << " last_snapshot_term " << saved_last_snapshot_term;
    }
    ...
    if (snapshot_save_done) {
      snapshot_save_done->start(meta);
    }
    ...
    if (_fsm_caller->on_snapshot_save(snapshot_save_done) != 0) {
        ...
        LOG(ERROR)<<"The raft node is down";
        return;
    }
    _running_jobs.add_count(1);

    LOG(INFO) << "node " << (_node != NULL ? _node->node_id().to_string() : "NULL")
        << " do snapshot with meta running, meta " << meta.DebugString()
        << " last_snapshot_index " << saved_last_snapshot_index
        << " last_snapshot_term " << saved_last_snapshot_term;
}
...
int SnapshotExecutor::on_snapshot_save_done(
    ...
    if (ret == 0) {
        if (writer->save_meta(meta)) {
            ...
            LOG(INFO) << "node " << _node->node_id() << " save snapshot write meta success";
        }
    } else {
        if (writer->ok()) {
            writer->set_error(ret, "Fail to do snapshot");
            LOG(ERROR)<<"on_snapshot_save_done Fail to do snapshot";
        }
    }
    ...
    if (ret == EIO) {
        ...
        LOG(ERROR) << ss.str() <<" Fail to save snapshot";
    }
    _saving_snapshot = false;
    lck.unlock();
...
int SnapshotExecutor::init(const SnapshotExecutorOptions& options) {
    ...
    LOG(INFO) << "Init snapshot storage success, uri " << options.uri;
...
void SnapshotExecutor::load_downloading_snapshot(DownloadingSnapshot* ds,
    ...
    if (ret != 0) {
        ...
        LOG(INFO) << "node " << _node->node_id() << " call on_snapshot_load success";
    }
}
