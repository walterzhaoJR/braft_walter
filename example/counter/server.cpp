...
DEFINE_bool(is_learner, false, "is learner node");
...
public:
    ...
    int start() {
        ...
        node_options.is_learner = FLAGS_is_learner;
        braft::Node* node = new braft::Node(FLAGS_group, braft::PeerId(addr));
        bool can_elec_ld = FLAGS_is_learner ? false : true;
        LOG(INFO) << "is learner:" << FLAGS_is_learner << " so elec_self:" << can_elec_ld;
        if (node->init(node_options, can_elec_ld) != 0) {
