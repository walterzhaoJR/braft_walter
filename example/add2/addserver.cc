...
public:
	AddStateMachine():_node(NULL),leader_term_(-1),id_(-1){};
    ...
    void redirect(AddResponse* response) {
        ...
        if (_node) {
            ...
            if (!leader.is_empty()) {
            	LOG(DEBUG) << "id:"<<id_<<"," <<"redirect : " << leader;
                response->set_redirect(leader.to_string());
            }
            else{
            	LOG(ERROR) << "id:"<<id_<<"," <<"NO LEADER ";
            }
        }
...
void AddStateMachine::on_snapshot_save(::braft::SnapshotWriter* writer,
                                  ::braft::Closure* done){
	brpc::ClosureGuard done_guard(done);
	std::string snapshot_path = writer->get_path();
	snapshot_path.append("/data");
	std::ofstream os(snapshot_path.c_str());
	for (auto it = number_map_.begin(); it != number_map_.end() ; it++) {
...
int AddStateMachine::on_snapshot_load(::braft::SnapshotReader* reader){
	...
	while (is >> key >> value) {
		...
		number_map_[key] = value;
	}
	return 0;
}

int AddStateMachine::start(int id){
	id_ = id;
	butil::EndPoint addr(butil::my_ip(), FLAGS_port);
    braft::Node* node = new braft::Node(FLAGS_group, braft::PeerId(addr,id));
    braft::NodeOptions node_options;
    stringstream data_path_stream;
    data_path_stream << FLAGS_data_path << id;
    string data_path = data_path_stream.str();
    if (!butil::CreateDirectory(butil::FilePath(data_path))) {
        LOG(ERROR) << "Fail to create directory " << data_path;
        return -1;
    }
    ...
    stringstream ss(FLAGS_conf);
    string t;
	stringstream conf;
	while (getline(ss, t, ',')) {
		conf << t <<":"<<id<<",";
	}
	 LOG(INFO) << "CONF:"<<conf.str();
    if (node_options.initial_conf.parse_from(conf.str()) != 0) {
        LOG(ERROR) << "Fail to parse configuration `" << conf.str() << '\'';
        return -1;
    }
...
void AddStateMachine::write(const ::example::AddRequest* request,::example::AddResponse* response
	...
	LOG(INFO)  << "id:"<<id_<<","<<"request ptr:" <<request;
	LOG(INFO)  << "id:"<<id_<<","<<"done ptr:" <<done;
	LOG(INFO) << "id:"<<id_<<"," << "request message ,key:" << request->key() << ",number:" << request->number();
...
public:
	AddServiceImpl(AddStateMachine* sm,AddStateMachine* sm2):statemachine_(sm),statemachine2_(sm2)
		,statemachine3_(NULL){

	}

	void shutdown(){
		if (statemachine3_ != NULL) {
			statemachine3_->shutdown();
		}
	}
	...
	void write(::google::protobuf::RpcController* controller,
		...
		brpc::ClosureGuard done_guard(done);
		if (request->key() == "key2") {
			statemachine2_->write(request,response, done_guard.release());
			return;
		}
		if (request->key() == "key3") {
			if (statemachine3_ == NULL) {
				response->set_success(false);
				response->set_result(-1);
				response->set_redirect("statemachine3 can't start.");
				return;
			}
			statemachine3_->write(request,response, done_guard.release());
			return;
		}
	...
	void start_new_statemachine(::google::protobuf::RpcController* controller,
	                       const ::example::NewStateMachineRequest* request,
	                       ::example::NewStateMachineResponse* response,
	                       ::google::protobuf::Closure* done){
		brpc::ClosureGuard done_guard(done);
		statemachine3_ = new AddStateMachine();
	    if (statemachine3_->start(2) != 0) {
	        LOG(ERROR) << "Fail to start state machine1";
	        response->set_success(false);
	        return ;
	    }
	    response->set_success(true);
...
private:
	AddStateMachine* statemachine_;
	AddStateMachine* statemachine2_;
	AddStateMachine* statemachine3_;
};
...
int main(int argc, char* argv[]){
    ...
    example::AddStateMachine statemachine2;
    example::AddServiceImpl service(&statemachine,&statemachine2);
    ...
    if (statemachine.start(0) != 0) {
        LOG(ERROR) << "Fail to start state machine0";
        return -1;
    }

    if (statemachine2.start(1) != 0) {
        LOG(ERROR) << "Fail to start state machine1";
        return -1;
    }
